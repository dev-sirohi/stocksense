...
export default function AskVikram() {
    ...
    const sendMessage = async (question: string) => {
        ...
        try {
            ...
            const response = await fetch(`/api/inventory/ask?q=${encodeURIComponent(question)}`);
    ...
    return (
        <div className="flex flex-col h-screen max-h-screen p-8 gap-4">
            <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Ask anything about your inventory in plain English.</p>
            </div>
                ...
                {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                        ...
                        >
                            ...
                            {msg.streaming && <span className="cursor-blink text-accent font-bold">▍</span>}
                        </div>
                    </div>
                ))}
                <div ref={bottomRef} />
            ...
            <form onSubmit={handleSubmit} className="flex gap-3">
                ...
                />
                <button type="submit" disabled={streaming || !input.trim()} className="btn-primary rounded-xl px-5">
                    {streaming ? (
