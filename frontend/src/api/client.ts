...
export const fetchAlerts = () => api.get<AlertsResponse>('/inventory/alerts').then((r) => r.data);

export const fetchSKUs = (params: { category?: string; skip?: number; limit?: number }) =>
    api.get<SKUListResponse>('/inventory/skus', { params }).then((r) => r.data);

export const fetchCategories = () => api.get<{ categories: string[] }>('/inventory/categories').then((r) => r.data);
